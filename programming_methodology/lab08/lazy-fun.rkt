...
(define (eval-env e ρ)
  (match e
    ...
    [(var-expr x) ;(eval-env (env-lookup ρ x) ρ)]
      (let ((ev (env-lookup ρ x)))
        (eval-env (car ev) (cdr ev)))]
    [(fun x e)    (clo x e ρ)]
    [(app e1 e2)
     (let* ((v1 (eval-env e1 ρ)))
       (match v1
         [(clo x e ρ) (eval-env e (env-add ρ x (cons e2 ρ)))]
    ...
    [(fst e)
     (let ((ev (eval-env e ρ)))
       (match (car ev)
         [(pair-expr el er)
          (eval-env el (cdr ev))]
         [_ (raise (type-error 'pair (car ev)))]))] 
    [(snd e)
     (let ((ev (eval-env e ρ)))
       (match (car ev)
         [(pair-expr el er)
     ...
     ;(let ((v (eval-env e ρ)))
      ; (if (pair? v)
       ;    (eval-env (cdr v) ρ)
        ;   (raise (type-error 'pair v))))]
    [(let-expr x e1 e2)
       (eval-env e2 (env-add ρ x (cons e1 ρ)))]))
...
(eval (parse '(let [y 0] (let [f (lambda (x) (let [y 2] (+ x y)))] (f (+ y 1))))))
; expected : 3

(eval (parse '(let [y 0] (let [p (pair y y)] (let [y 10] (fst p))))))
; expected : 0
