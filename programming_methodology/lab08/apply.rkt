...
(struct apply-expr     (fun args)      #:transparent) ; <---------------------!
...
(define (eval-env e ρ)
  (match e
    ...
    [(apply-expr fun args)
     (eval-env (make-app fun (map (lambda (x) (const x)) (eval-env args ρ))) ρ)]    ; <---------------------------------!
     [(pair-expr el er)
...
(define (parse e)
  (cond
   ...
   [(and (list? e)
         (= (length e) 3)
         (eq? (first e) 'apply))
    (apply-expr (parse (second e)) (parse (third e)))] ; <-------------------!
   [(and (list? e)
         (> (length e) 1))
...
; TESTY
(eval (parse '(apply (lambda (x y) (+ x y)) (let (x 2) (list 1 x)))))
; expected : 3
